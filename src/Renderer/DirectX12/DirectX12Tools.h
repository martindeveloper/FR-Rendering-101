...
public:
    ...
    static void LoadShaderByteCode(const wchar_t *path, ID3DBlob **byteCode)
    {
        ...
        {
            Platform::GetLogger()->Fatal("RendererDirectX12::LoadShaderByteCode: File %s does not exist", path);
        ...
        if (FAILED(result))
        {
            Platform::GetLogger()->Fatal("RendererDirectX12::LoadShaderByteCode: Failed to create blob for file %s", path);
            Platform::TriggerCrash();
        }
    ...
    static void CheckShaderError(HRESULT result, ID3DBlob *blob, const char *message, bool shouldCrash)
    {
        if (FAILED(result))
        {
            Platform::GetLogger()->Fatal("TriangleEntity: %s", message);

            if (blob != nullptr)
            {
                Platform::GetLogger()->Fatal("TriangleEntity: %s", reinterpret_cast<const char *>(blob->GetBufferPointer()));
            }
